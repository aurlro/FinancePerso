...
def apply_migration():
    ...
    try:
        ...
        new_content = beta_content.replace(
            'page_title="Dashboard Beta V5.5"',
            'page_title="Synthèse"'
        ).replace(
            'page_icon="✨"',
            'page_icon="📊"'
        ).replace(
            'st.sidebar.title("💰 Dashboard Beta V5.5")',
...
def main():
    parser = argparse.ArgumentParser(
        description="Migration vers Dashboard V5.5"
    )
    parser.add_argument(
        '--check',
        action='store_true',
        help="Vérifie l'état de la migration (défaut)"
    )
    parser.add_argument(
        '--apply',
        action='store_true',
        help="Applique la migration"
    )
    parser.add_argument(
        '--rollback',
        action='store_true',
        help="Annule la migration"
    )
