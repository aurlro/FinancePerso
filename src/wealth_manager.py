...
Usage:
    from src.wealth_manager import WealthManager, AssetType
...
class WealthManager:
    ...
    def get_total_net_worth(self) -> float:
        ...
        non_mortgage_debt = sum(
            l.remaining_amount for l in self.liabilities
    ...
    def get_liquidity_analysis(self) -> Dict[str, float]:
        ...
        medium_term = sum(
            a.current_value for a in self.financial_assets
            if a.liquidity in (AssetLiquidity.IMMEDIATE, AssetLiquidity.SHORT_TERM, AssetLiquidity.MEDIUM_TERM)
        )
    ...
    def get_summary(self) -> str:
        ...
        for category, data in allocation.items():
            if category != 'other_debt' and data['amount'] > 0:
                emoji = {'cash': '💰', 'real_estate': '🏠', 'financial': '📈', 'crypto': '₿'}.get(category, '📊')
