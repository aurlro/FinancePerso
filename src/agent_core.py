...
Usage:
    from src.agent_core import AgentOrchestrator, TriggerType
...
from src.subscription_engine import Subscription, SubscriptionStatus
from src.wealth_manager import (
    WealthManager, AssetType, LiabilityType, 
    FinancialAsset, RealEstateAsset, CryptoAsset
)
...
class TriggerDetector:
    ...
    def detect_better_alternatives(
        ...
        for sub in subscriptions:
            ...
            if any(x in merchant_upper for x in ['SFR', 'ORANGE', 'FREE', 'BOUYGUES']):
                ...
                category = 'STREAMING'
            elif any(x in merchant_upper for x in ['FREE', 'ORANGE', 'SFR']) and 'BOX' in merchant_upper:
                category = 'INTERNET'
    ...
    def detect_debt_optimization(
        ...
        total_monthly_payments = sum(
            l.monthly_payment for l in wealth_manager.liabilities
        )
...
class AgentOrchestrator:
    ...
    def get_mission_summary(self, missions: List[Mission]) -> Dict[str, Any]:
        ...
        return {
            ...
            'by_type': by_type,
            'high_impact_missions': [m for m in missions if m.priority in (
