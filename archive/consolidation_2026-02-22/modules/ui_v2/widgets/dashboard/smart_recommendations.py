...
Recommandations intelligentes pour le tableau de bord Budgets.
"""
    ...
    with st.container(border=True):
        ...
        with col3:
            ...
            with action_cols[0]:
                if st.button(
                    "🔍 Vérifier",
                    key=f"check_{insight.category}_{insight.insight_type}",
...
def _render_trend_card(insight: CategoryInsight):
    ...
    with st.container(border=True):
        ...
        with col2:
            icon = (
                "📈"
                if "augmenté" in insight.description
                else "📉" if "réduit" in insight.description else "📊"
            )
            st.markdown(f"### {icon}")
...
def _render_suggestion_card(insight: CategoryInsight):
    ...
    with st.container(border=True):
        ...
        with col3:
            st.markdown("### 💡")
            if st.button("En savoir +", key=f"sugg_{insight.category}", use_container_width=True):
