...
Form for manually setting up financial profile.

Migrated from: modules/ui/components/profile_form.py
Classification: Molecule (form composite)
"""
...
def render_profile_setup_form(key_prefix="profile"):
    ...
    with st.form(f"{key_prefix}_form"):
        ...
        with col1:
            salary_employer = st.text_input(
                "Employeur / Source (Mot-clé)",
                placeholder="Ex: CAPGEMINI, CAF...",
                key=f"{key_prefix}_salary_kw",
            )
        with col2:
            salary_amount = st.number_input(
                "Montant Net Mensuel (€)", min_value=0.0, step=50.0, key=f"{key_prefix}_salary_amt"
            )
        ...
        with col3:
            housing_keyword = st.text_input(
                "Bailleur / Banque (Mot-clé)",
                placeholder="Ex: FONCIA, CREDIT AGRICOLE...",
                key=f"{key_prefix}_housing_kw",
            )
        with col4:
            housing_amount = st.number_input(
                "Loyer / Mensualité (€)", min_value=0.0, step=50.0, key=f"{key_prefix}_housing_amt"
            )

        # --- ABONNEMENTS ---
        st.markdown("### 📱 Abonnements & Charges")
        st.caption(
            "Ajoutez des mots-clés pour vos charges récurrentes (électricité, internet, téléphone...)"
        )
        ...
        with c_sub1:
            sub1_kw = st.text_input(
                "Abonnement 1 (Mot-clé)", placeholder="Ex: EDF, ENGIE...", key=f"{key_prefix}_sub1"
            )
        with c_cat1:
            sub1_cat = st.selectbox(
                "Catégorie",
                cats,
                index=cats.index("Logement") if "Logement" in cats else 0,
                key=f"{key_prefix}_cat1",
            )

        with c_sub2:
            sub2_kw = st.text_input(
                "Abonnement 2 (Mot-clé)",
                placeholder="Ex: ORANGE, NETFLIX...",
                key=f"{key_prefix}_sub2",
            )
        with c_cat2:
            sub2_cat = st.selectbox(
                "Catégorie",
                cats,
                index=cats.index("Abonnements") if "Abonnements" in cats else 0,
                key=f"{key_prefix}_cat2",
            )

        with c_sub3:
            sub3_kw = st.text_input(
                "Abonnement 3 (Mot-clé)",
                placeholder="Ex: MAIF, ALIANZ...",
                key=f"{key_prefix}_sub3",
            )
        with c_cat3:
            sub3_cat = st.selectbox(
                "Catégorie",
                cats,
                index=cats.index("Assurances") if "Assurances" in cats else 0,
                key=f"{key_prefix}_cat3",
            )
