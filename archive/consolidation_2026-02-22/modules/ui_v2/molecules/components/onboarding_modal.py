...
Migrated from: modules/ui/components/onboarding_modal.py
Classification: Molecule (multi-step modal dialog)
"""
...
def render_onboarding_modal():
    ...
    def show_modal():
        ...
        if step == 1:
            ...
            with col2:
                if st.button(
                    "🚀 Commencer", type="primary", use_container_width=True, key="button_77"
                ):
        ...
        # --- STEP 2: Members ---
            ...
            if not members.empty:
                st.success(
                    f"✅ {len(members)} membre(s) déjà configuré(s) : {', '.join(members['name'].tolist())}"
                )

            with st.form("add_member_form"):
                new_member = st.text_input(
                    "Nom du membre", placeholder="Ex: Aurélien, Élise...", key="text_input_94"
                )
                member_type = st.selectbox(
                    "Type",
                    options=["HOUSEHOLD", "EXTERNAL"],
                    format_func=lambda x: (
                        "Membre du foyer" if x == "HOUSEHOLD" else "Extérieur (tiers)"
                    ),
        ...
        # --- STEP 3: Categories ---
            ...
            custom_cat = st.text_input(
                "Ou créez une catégorie personnalisée",
                placeholder="Ex: Sport, Enfants...",
                key="text_input_167",
            )
            ...
            with col1:
                if st.button(
                    "➕ Ajouter les catégories", use_container_width=True, key="button_171"
                ):
        ...
        # --- STEP 4: Optional Settings ---
            ...
            with col2:
                if st.button(
                    "🚀 C'est parti !", type="primary", use_container_width=True, key="button_261"
                ):
                    dismiss_onboarding()
                    st.rerun()
