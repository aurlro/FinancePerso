...
def find_similar_transactions(
    transaction_id: int, df: pd.DataFrame = None, similarity_threshold: float = 0.8
) -> pd.DataFrame:
    """
    ...
    with container:
        ...
        with selector_col:
            selected = st.multiselect(
                ...
                placeholder=f"{IconSet.TAG.value} Ajouter des tags...",
            )

        with add_col:
            with st.popover(IconSet.ADD.value, use_container_width=True):
            ...
            if new_tags:
                ...
                if not similar_df.empty:
                    ...
                    for _, tx in similar_df.iterrows():
                        tx_tags = [
                            t.strip() for t in str(tx.get("tags", "")).split(",") if t.strip()
                        ]
                    ...
                    if missing_tags_tx:
                        ...
                        for tx_info in missing_tags_tx[:5]:  # Show top 5
                            ...
                            with tx_col3:
                                ...
                                if st.button(f"{IconSet.TAG.value} Taguer", key=prop_key, use_container_width=True):
                                    # Add tags to this transaction
                                    if tx_info["id"] not in st.session_state.get(
                                        "pending_tag_additions", {}
                                    ):
                                        st.session_state["pending_tag_additions"][
                                            tx_info["id"]
                                        ] = []
                        ...
                        if len(missing_tags_tx) > 1:
                            if st.button(
                                f"{IconSet.TAG.value} Taguer toutes les {len(missing_tags_tx)} transactions",
                                type="primary",
                                use_container_width=True,
                                ...
                                st.toast(
                                    f"✅ Tags propagés à {len(missing_tags_tx)} transactions !",
                                    icon="🚀",
                                )
                                st.rerun()
                    else:
