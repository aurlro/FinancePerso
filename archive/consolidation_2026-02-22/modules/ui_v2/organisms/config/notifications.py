...
Provides UI for configuring notification settings with preview mode.
"""
...
def render_notification_settings() -> None:
    ...
    preview: Dict[str, Any] = st.session_state["notif_preview"]
    ...
    if not preview["notif_enabled"]:
        st.info(f"{IconSet.BELL.value} Les notifications sont désactivées.")
        if st.button(f"{IconSet.SAVE.value} Sauvegarder", type="primary", key="notif_save_disabled"):
            _save_all_settings(preview)
            st.success(f"{IconSet.SUCCESS.value} Paramètres sauvegardés !")
            st.rerun()
        return

    # --- DESKTOP NOTIFICATIONS ---
    st.divider()
    st.subheader(f"{IconSet.COMPUTER.value} Notifications Desktop")
    ...
    if preview["notif_desktop"]:
        st.caption(f"{IconSet.LIGHTBULB.value} Les notifications desktop fonctionnent sur macOS, Linux et Windows.")

    # --- EMAIL NOTIFICATIONS ---
    st.divider()
    st.subheader(f"{IconSet.EMAIL.value} Notifications Email")
    ...
    if preview["notif_email_enabled"]:
        ...
        if not email_configured:
            st.warning(f"{IconSet.WARNING.value} Configuration email incomplète - remplissez tous les champs SMTP")
        ...
        with st.container(border=True):
            ...
            with col1:
                preview["notif_smtp_server"] = st.text_input(
                    "Serveur SMTP", value=preview["notif_smtp_server"], placeholder="smtp.gmail.com"
                )
            ...
            with col2:
                preview["notif_smtp_port"] = st.number_input(
                    "Port", value=int(preview["notif_smtp_port"]), min_value=1, max_value=65535
                )
            ...
            st.info(
                f"""
            {IconSet.PIN.value} **Note pour Gmail :**
    ...
    # --- ALERT THRESHOLDS ---
    st.divider()
    st.subheader(f"{IconSet.ALERT.value} Seuils d'alerte")
    ...
    with col_t3:
        preview["notif_threshold_notice"] = st.slider(
            f"{IconSet.INFO.value} Information",
            min_value=50,
            max_value=80,
    ...
    with col_save:
        if st.button(
            f"{IconSet.SAVE.value} Sauvegarder les paramètres",
            type="primary",
            use_container_width=True,
            key="notif_save_settings",
        ):
            _save_all_settings(preview)
            toast_success("Paramètres de notification sauvegardés", icon=IconSet.BELL.value)
            st.success(f"{IconSet.SUCCESS.value} Paramètres sauvegardés !")
            st.rerun()

    with col_test1:
        if st.button(f"{IconSet.EXPORT.value} Test notification", use_container_width=True, key="notif_test_send"):
            ...
            with st.spinner("Envoi en cours..."):
                ...
                if results["desktop"]:
                    st.success(f"{IconSet.SUCCESS.value} Desktop OK !")
                    toast_success("Notification desktop envoyée", icon=IconSet.COMPUTER.value)
                if results["email"]:
                    st.success(f"{IconSet.SUCCESS.value} Email OK !")
                    toast_success("Email de test envoyé", icon=IconSet.EMAIL.value)
                ...
                if results["errors"]:
                    for error in results["errors"]:
                        ...
                        if "Authentification" in error:
                            st.info(
                                f"{IconSet.LIGHTBULB.value} **Pour Gmail:** Utilisez un 'App Password' généré sur https://myaccount.google.com/apppasswords"
                            )
                        elif "connecter" in error.lower():
                            st.info(
                                f"{IconSet.LIGHTBULB.value} Vérifiez que le serveur SMTP et le port sont corrects (Gmail: smtp.gmail.com:587)"
                            )
                        elif "Destinataire" in error:
                            st.info(f"{IconSet.LIGHTBULB.value} Vérifiez que l'adresse email destinataire est valide")

                if not results["desktop"] and not results["email"] and not results["errors"]:
                    st.warning(f"{IconSet.WARNING.value} Aucune notification activée")
                    toast_warning("Aucune notification activée", icon=IconSet.BELL.value)

    with col_test2:
        if st.button(f"{IconSet.BELL.value} Vérifier budgets", use_container_width=True, key="notif_check_budgets"):
            _save_all_settings(preview)
            with st.spinner("Analyse..."):
                alerts = check_budget_alerts(force_check=True)

                if alerts:
                    st.success(f"{IconSet.ALERT.value} {len(alerts)} alerte(s) !")
                    toast_warning(f"{len(alerts)} alerte(s) budget détectée(s)", icon=IconSet.ALERT.value)
                    for alert in alerts:
                        emoji = (
                            IconSet.ALERT.value
                            if alert["level"] == "critical"
                            else IconSet.WARNING.value if alert["level"] == "warning" else IconSet.INFO.value
                        )
                        st.write(
                            f"{emoji} **{alert['category']}**: {alert['spent']:.0f}€ / {alert['budget']:.0f}€"
                        )
                else:
                    st.info(f"{IconSet.SUCCESS.value} Pas d'alerte")
                    toast_info("Aucune alerte budget", icon=IconSet.SUCCESS.value)

    # --- ALERT HISTORY ---
    st.divider()
    st.subheader(f"{IconSet.HISTORY.value} Historique des alertes")
    ...
    if alert_count > 0:
        st.write(f"{IconSet.CHART.value} **{alert_count}** alerte(s) envoyée(s) ce mois-ci")
    else:
        st.write(f"{IconSet.CHART.value} Aucune alerte ce mois-ci")


def _save_all_settings(preview: Dict[str, Any]) -> None:
