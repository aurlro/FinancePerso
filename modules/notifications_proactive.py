    ...
    try:
        ...
        if current_spend > avg_spend * 1.2:  # 20% increase
            ...
            return ProactiveAlert(
                ...
                action=lambda: st.switch_page("pages/3_Synthèse.py")
            )
    ...
    try:
        ...
        if len(duplicates) > 0:
            ...
            return ProactiveAlert(
                ...
                action=lambda: st.switch_page("pages/1_Opérations.py")
            )
    except Exception as e:
...
def detect_missing_salary() -> ProactiveAlert | None:
    ...
    try:
        ...
        salary_patterns = df[
            (df['amount'] > 1000) & 
            (df['label'].str.contains('SALAIRE|PAIE|VIREMENT.*EMPLOI', case=False, na=False, regex=True))
        ]
    ...
    try:
        ...
        if category_variance:
            ...
            return ProactiveAlert(
                ...
                action=lambda: st.switch_page("pages/5_Budgets.py")
            )
    except Exception as e:
...
def detect_recurring_payment_missing() -> ProactiveAlert | None:
    ...
    try:
        ...
        recurring_candidates = recent_df[
            recent_df['label'].str.contains('EDF|ORANGE|SFR|FREE|SPOTIFY|NETFLIX|ASSURANCE|LOYER', 
                                           case=False, na=False)
        ]
        ...
        current_month_transactions = set(
            recurring_candidates[recurring_candidates['month'] == current_month]['label'].str.upper()
        )
        ...
        for label in prev_transactions['label'].unique():
            ...
            if not any(label.upper() in curr for curr in current_month_transactions):
                ...
                two_month_data = recurring_candidates[recurring_candidates['month'] == two_months_ago]
