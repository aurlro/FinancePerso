            ...
            if idx < len(categories_sorted):
                ...
                with col:
                    if st.button(
                        ...
                        launch_explorer("category", raw_cat, "07_Recherche")


def render_category_pie_chart(
    df_current: pd.DataFrame, cat_emoji_map: dict, title: str = "Répartition des Dépenses"
):
    """
...
def _prepare_monthly_stacked_data(df: pd.DataFrame, cat_emoji_map: dict) -> pd.DataFrame:
    ...
    df_monthly["display_category"] = df_monthly.apply(
        lambda x: (lambda v: f"{cat_emoji_map.get(v, '🏷️')} {v}")(
            x["category_validated"]
            if x["category_validated"] != "Inconnu"
            else (x["original_category"] or "Inconnu")
        ),
    ...
    # Group and aggregate
    df_stacked = (
        df_monthly.groupby(["month_year", "display_category"])["amount"].sum().reset_index()
    )
...
def render_monthly_stacked_chart(df: pd.DataFrame, cat_emoji_map: dict):
    ...
    if not df_revenus_mensuel.empty:
        ...
        df_combined = df_revenus_mensuel.merge(df_depenses_totales, on="Mois", how="outer").fillna(
            0
        )
