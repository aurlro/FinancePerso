...
def get_filter_options(df: pd.DataFrame) -> Dict:
    ...
    accounts = (
        sorted(df["account_label"].unique().tolist()) if "account_label" in df.columns else []
    )
...
def normalize_name(name: str) -> str:
    ...
    if not name or pd.isna(name):
        return ""
    return "".join(
        c for c in unicodedata.normalize("NFD", str(name)) if unicodedata.category(c) != "Mn"
    ).lower()
...
def render_filter_sidebar(df: pd.DataFrame) -> Dict:
    ...
    if "account_label" in df_filtered.columns and not df_filtered.empty:
        ...
        selected_accounts = st.sidebar.multiselect(
            "Comptes", accounts, default=accounts, key="filter_accounts"
        )
    ...
    if "member" in df_filtered.columns and not df_filtered.empty:
        ...
        selected_members = st.sidebar.multiselect(
            "Membres (Carte)", members, default=members, key="filter_members"
        )
        if selected_members:
    ...
    if "beneficiary" in df_filtered.columns and not df_filtered.empty:
        df_filtered["beneficiary_display"] = consolidate_names(
            df_filtered["beneficiary"], official_list
        )
        beneficiaries = sorted(df_filtered["beneficiary_display"].unique().tolist())
        selected_benefs = st.sidebar.multiselect(
            "Bénéficiaires", beneficiaries, default=beneficiaries, key="filter_beneficiaries"
        )
        if selected_benefs:
    ...
    if "tags" in df_filtered.columns:
        ...
        if all_available_tags:
            selected_tags = st.sidebar.multiselect(
                "Filtrer par Tags 🏷️", all_available_tags, key="filter_tags"
            )
            if selected_tags:
    ...
    show_internal = st.sidebar.checkbox(
        "Afficher virements internes 🔄", value=False, key="filter_show_internal"
    )
    show_hors_budget = st.sidebar.checkbox(
        "Afficher hors budget 🚫", value=False, key="filter_show_hors_budget"
    )
...
def render_filter_info(df_full: pd.DataFrame, filter_result: Dict):
    ...
    if not df_full.empty:
        ...
        if not important_excluded.empty:
            ...
            with st.expander(
                f"⚠️ **{count} opérations majeures** sont exclues par vos filtres", expanded=False
            ):
                st.warning(
                    f"Dernière exclue : **{latest['label']}** ({latest['amount']}€) le {latest['date']}."
                )
                st.caption(
                    "Ajustez vos filtres (Année, Comptes, Hors Budget) pour les inclure dans vos analyses."
                )
