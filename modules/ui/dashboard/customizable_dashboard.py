...
LAYOUT_TEMPLATES = {
    ...
    "analytique": {
        ...
        "widgets": [
            ...
            DashboardWidget("month_1", WidgetType.MONTHLY_STACKED, "📅 Évolution mensuelle", 9, "large"),
        ]
    },
    "budget": {
        "name": "🎯 Budget",
        ...
        "widgets": [
            ...
            DashboardWidget("rec_1", WidgetType.SMART_RECOMMENDATIONS, "💡 Recommandations", 5, "medium"),
            DashboardWidget("cat_1", WidgetType.CATEGORIES_CHART, "📊 Répartition", 6, "medium"),
        ]
    },
    "complet": {
        ...
        "widgets": [
            ...
            DashboardWidget("rec_1", WidgetType.SMART_RECOMMENDATIONS, "💡 Recommandations", 12, "medium"),
        ]
    }
}
...
def render_dashboard_configurator():
        ...
        with template_cols[i]:
            with st.container(border=True):
                ...
                if st.button("Appliquer", key=f"template_{template_key}", use_container_width=True):
                    ...
                    st.session_state[manager.preview_key] = [w.to_dict() for w in template_data['widgets']]
    ...
    with col1:
        ...
        if saved_layouts:
            ...
            if layout_names:
                selected = st.selectbox("Charger", ["Sélectionner..."] + layout_names, label_visibility="collapsed")
                if selected != "Sélectionner..." and st.button("Charger", use_container_width=True, key="load_saved"):
    ...
    def flush_row():
        ...
        if len(current_row_widgets) == 1:
            ...
            for col, widget in zip(cols, current_row_widgets):
                with col:
                    _render_widget_content(widget, df_current, df_prev, df_full, cat_emoji_map, trends, in_grid=True)
...
def _render_widget_content(widget, df_current, df_prev, df_full, cat_emoji_map, trends, in_grid=False):
    ...
    with container:
        ...
        if widget.type == WidgetType.EVOLUTION_CHART and not df_current.empty:
            ...
            df_exp = prepare_expense_dataframe(df_current, cat_emoji_map) if not df_current.empty else pd.DataFrame()
            ...
            if not budgets.empty:
                ...
                df_month = df_full[df_full['date_dt'].dt.strftime('%Y-%m') == current_month] if df_full is not None else pd.DataFrame()
