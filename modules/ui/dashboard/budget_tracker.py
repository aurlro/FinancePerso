...
def render_budget_tracker(df_exp: pd.DataFrame, cat_emoji_map: dict, df_full: pd.DataFrame = None):
    ...
    if budgets.empty:
        ...
        st.caption("Configurez vos budgets pour suivre vos dépenses et recevoir des alertes intelligentes.")
    ...
    historical_map = {}
    if df_full is not None and 'date_dt' in df_full.columns and 'category_validated' in df_full.columns:
        try:
    ...
    with col_summary3:
        ...
        st.metric("Reste à dépenser", f"{remaining:+.0f}€",
        ...
        with cols_b[col_idx]:
            with st.container(border=True):
                ...
                if budget['percent'] > 100:
                    ...
                    st.error(f"🔴 **+{overrun:.0f}€** dépassé ({budget['spent']:.0f}/{budget['limit']:.0f}€)")
                    ...
                    st.info(f"**{budget['delta']:.0f}€** restant ({budget['spent']:.0f}/{budget['limit']:.0f}€)")
                else:
                    st.progress(percent_normalized, text=f"{budget['percent']:.0f}%")
                ...
                if budget['percent'] > 90:
                    ...
                    with cols_action[0]:
                        if st.button("💡 Conseils", key=f"tip_{budget['cat_name']}", use_container_width=True):
