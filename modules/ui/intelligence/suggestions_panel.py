...
def _handle_suggestion_action(suggestion: Suggestion):
    ...
    try:
        if action_type == 'create_rule':
            ...
            toast_info(f"💳 Allez dans Configuration → Membres pour mapper la carte ...{card_suffix}", icon="👤")
            return True
            ...
            toast_info(f"🏦 Allez dans Configuration → Membres pour définir le membre par défaut", icon="👤")
            return True
            ...
            toast_info(f"🏷️ Pour fusionner '{source}' → '{target}', utilisez Configuration → Catégories", icon="🏷️")
            return True
...
@st.fragment
def render_smart_suggestions_panel(transactions_df: pd.DataFrame,
    ...
    for i, suggestion in enumerate(filtered_suggestions[:10]):  # Limit to 10
        with st.container(border=True):
            ...
            with impact_col:
                st.progress(suggestion.impact_score / 100, 
                           text=f"Impact: {suggestion.impact_score}/100")
            
            with action_col:
                action_key = f"sugg_action_{suggestion.id}_{i}"
                
                if suggestion.auto_fixable:
                    if st.button(f"✨ {suggestion.action_label}", 
                    ...
                    if st.button(suggestion.action_label, 
                                use_container_width=True, 
                                key=action_key):
...
def render_mini_suggestions_card(transactions_df: pd.DataFrame,
                                  rules_df: pd.DataFrame,
                                  budgets_df: pd.DataFrame,
                                  members_df: pd.DataFrame):
    """
    ...
    suggestions = get_smart_suggestions(transactions_df, rules_df, budgets_df, members_df)
    suggestions = [s for s in suggestions if s.id not in st.session_state.get('suggestions_dismissed', set())]
