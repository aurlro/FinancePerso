...
def render_loans_summary():
    ...
    st.progress(summary['average_progress'] / 100, text=f"Remboursement global: {summary['average_progress']:.1f}%")
...
def render_loan_editor(loan: dict):
    ...
    with st.form(key=f"loan_form_{loan['id']}"):
        ...
        with cols[0]:
            ...
            lender = st.text_input("Organisme", value=loan.get('lender', '') or '', key=f"loan_lender_{loan['id']}")
        ...
        with member_col:
            member_id = st.selectbox(
                ...
                index=member_options.index(current_member) if current_member in member_options else 0,
        ...
        with type_col:
            account_type = st.selectbox(
                ...
                index=['JOINT', 'PERSONAL_A', 'PERSONAL_B'].index(loan.get('account_type', 'JOINT')),
                key=f"loan_type_{loan['id']}"
            )
        
        notes = st.text_area("Notes", value=loan.get('notes', '') or '', key=f"loan_notes_{loan['id']}")
