...
def render_couple_setup():
    ...
    if progress["is_complete"]:
        st.success(
            "✅ Configuration couple terminée ! Le dashboard est disponible dans la Synthèse."
        )
...
def _render_current_user_setup():
    ...
    selected = st.radio(
        ...
        index=(
            0
            if current == settings["member_a_id"]
            else 1 if current == settings["member_b_id"] else 0
        ),
...
def _render_card_editor(mapping: dict):
    ...
    with cols[1]:
        ...
        member_labels = {
            None: "— Aucun —",
            **{m["id"]: f"{m['emoji']} {m['name']}" for m in members},
        }
...
def _render_card_configurator(card: dict, settings: dict):
    ...
    with cols[2]:
        ...
        label = st.text_input("Libellé", value=default_label, key=f"config_label_{card_suffix}")
