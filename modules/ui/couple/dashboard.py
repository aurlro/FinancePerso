...
def render_setup_prompt():
    ...
    st.write("Le mode Couple nécessite une configuration initiale pour attribuer les cartes et définir la confidentialité.")
...
def render_comparison_section(data: dict):
    ...
    partner_label = settings.get('member_b_name' if current_role == "A" else 'member_a_name', 'Partenaire')
        ...
        for transfer in pending:
            with st.container(border=True):
                ...
                with cols[3]:
                    ...
                    with val_cols[0]:
                        if st.button("✅", key=f"validate_{transfer['from_id']}_{transfer['to_id']}"):
...
def render_couple_summary_embedded():
    ...
    if st.button("📊 Voir le dashboard complet", key="goto_couple_dashboard", use_container_width=True):
