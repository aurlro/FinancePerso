...
def render_recurrence_card(
    ...
    with btn_cols[1]:
        if existing_feedback and existing_feedback["user_feedback"] == "rejected":
            if st.button(
                "❌ Rejetée", key=f"btn_rej_{label[:30]}", type="primary", use_container_width=True
            ):
                pass  # Already rejected
        else:
            if st.button(
                "❌ Pas une récurrence",
                key=f"btn_rej_{label[:30]}",
                type="secondary",
                use_container_width=True,
            ):
    ...
    with btn_cols[2]:
        if existing_feedback:
            if st.button(
                "🔄 Réinitialiser", key=f"btn_reset_{label[:30]}", use_container_width=True
            ):
    ...
    with st.form("manual_recurrence"):
        ...
        with col1:
            label_pattern = st.text_input(
                "Libellé ou pattern", placeholder="Ex: NETFLIX, SALAIRE, EDF..."
            )
        ...
        with col4:
            frequency = st.selectbox(
                "Fréquence", options=["Mensuel", "Trimestriel", "Annuel", "Variable"], index=0
            )

        notes = st.text_area(
            "Notes", placeholder="Pourquoi cette opération est récurrente...", height=80
        )
...
def render_batch_validation_actions(
    ...
    with col1:
        if st.button(
            "✅ Tout confirmer", type="primary", use_container_width=True, key="rec_confirm_all"
        ):
