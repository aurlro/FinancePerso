...
class ExploreCategoryAction(SuggestionAction):
    ...
    def execute(self, suggestion: Suggestion, action_data: dict) -> dict:
        ...
        category_txns = df[
            (df["category_validated"] == category) & (df["month"] == current_month)
        ].sort_values("amount")

        # Calculate comparison with average
        historical = df[df["month"] != current_month]
        avg_monthly = (
            historical[historical["category_validated"] == category]["amount"].sum() / 3
        )  # Approximation
        ...
        return {
            ...
            "view_data": {
                ...
                "difference_pct": (
                    ((abs(current_total) / abs(avg_monthly)) - 1) * 100 if avg_monthly else 0
                ),
            },
        }
        ...
        with st.container(border=True):
            ...
            with col1:
                if st.button("🔍 Voir toutes les transactions", key=f"explore_all_{category}"):
                    ...
                    st.session_state["active_intelligence_tab"] = (
                        "rules"  # Actually should navigate to operations
                    )
                    st.rerun()
            with col2:
...
class ViewSubscriptionAction(SuggestionAction):
    ...
    def execute(self, suggestion: Suggestion, action_data: dict) -> dict:
        ...
        monthly_evolution = (
            label_df.groupby("month").agg({"amount": ["mean", "count"]}).reset_index()
        )
                ...
                changes.append(
                    {
                        ...
                        "increase_pct": ((abs(curr["avg_amount"]) / abs(prev["avg_amount"])) - 1)
                        * 100,
                    }
                )
    ...
    def render_inline_view(self, view_data: dict) -> bool:
        ...
        with st.container(border=True):
            ...
            if monthly_data:
                ...
                st.dataframe(
                    df_display.rename(
                        columns={"month": "Mois", "avg_amount": "Montant moyen", "count": "Nombre"}
                    ),
                    hide_index=True,
        ...
        with st.container(border=True):
            ...
            if category:
                ...
                if not cat_txns.empty:
                    ...
                    if amount > p90:
                        st.warning(
                            f"⚠️ Cette transaction est supérieure au 90ème percentile ({p90:.0f}€)"
                        )
...
class MergeCategoriesAction(SuggestionAction):
    ...
    def render_inline_view(self, view_data: dict) -> bool:
        ...
        with st.container(border=True):
            ...
            with col1:
                if st.button(
                    "✅ Confirmer la fusion", key=f"merge_confirm_{source}_{target}", type="primary"
                ):
                    try:
    ...
    def render_inline_view(self, view_data: dict) -> bool:
        ...
        with st.container(border=True):
            ...
            for i, dup in enumerate(duplicates[:5]):  # Show first 5
                with st.expander(
                    f"🔍 {dup['date']} - {dup['label'][:40]}... " f"({dup['count']} occurrences)"
                ):
    ...
    def execute(self, suggestion: Suggestion, action_data: dict) -> dict:
        ...
        try:
            ...
            if matching.empty:
                return {
                    "success": False,
                    "message": f"❌ Aucune transaction trouvée pour '{label}'",
                }
            ...
            return {
                ...
                "view_data": {
                    ...
                    "transactions": matching[["date", "label", "amount", "category_validated"]]
                    .tail(6)
                    .to_dict("records"),
                },
            }
