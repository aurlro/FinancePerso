...
def _fetch_inbox_data() -> tuple:
    ...
    recurring_df = (
        detect_recurring_payments_v2(validated_df) if not validated_df.empty else pd.DataFrame()
    )
    ...
    try:
        ...
        if not recurring_df.empty:
            ...
            pending_mask = recurring_df.apply(
                lambda row: (row["label"], row.get("category", "")) not in feedback_map, axis=1
            )
            pending_recs = pending_mask.sum()

        # Filter out dismissed suggestions
        suggestions = [
            s for s in suggestions if s.id not in st.session_state.get("inbox_dismissed", set())
        ]
...
def _render_suggestions_section(standalone: bool = False):
    ...
    if active_suggestion_id:
        ...
        if active_suggestion:
            action_type = (
                active_suggestion.action_data.get("type")
                if isinstance(active_suggestion.action_data, dict)
                else None
            )
            if action_type in ("map_card", "map_account"):
                with st.container(border=True):
                    st.markdown("### 👤 Définir un membre par défaut")
                    _render_member_mapping_form(
                        active_suggestion, f"active_{active_suggestion.id}", "active_suggestion_id"
                    )
                st.divider()
...
def _render_suggestion_card(suggestion: Suggestion):
    ...
    with st.container(border=True):
        ...
        with cols[0]:
            ...
            st.markdown(
                f"<div style='font-size: 24px; text-align: center;'>{type_icons.get(suggestion.type, '💡')}</div>",
        ...
        with cols[2]:
            ...
            if st.button(btn_label, key=action_key, type=btn_type, use_container_width=True):
                ...
                action_type = (
                    suggestion.action_data.get("type")
                    if isinstance(suggestion.action_data, dict)
                    else None
                )
                if action_type in ("map_card", "map_account"):
        ...
        with cols[3]:
            ...
            if not st.session_state.get(confirm_key):
                if st.button(
                    "🗑️", key=f"sugg_dismiss_{suggestion.id}", help="Ignorer cette suggestion"
                ):
                ...
                with col1:
                    if st.button(
                        "Oui, ignorer", key=f"confirm_del_sugg_{suggestion.id}", type="primary"
                    ):
                        st.session_state.inbox_dismissed.add(suggestion.id)
                        ...
                        if (
                            st.session_state.get("active_suggestion_result", {}).get(
                                "suggestion_id"
                            )
                            == suggestion.id
                        ):
...
def _render_member_mapping_form(
    suggestion: Suggestion, card_key: str, active_state_key: str = None
):
    ...
    if show_new_member:
        ...
        if member_options:
            selected_member = st.selectbox(
                "Choisir un membre existant", options=member_options, key=f"{card_key}_member"
            )
        else:
