...
def render_rules_tab():
    ...
    st.caption("Définissez comment l'application catégorise et traite vos transactions automatiquement.")
        ...
        if len(matches) > 0:
            ...
            with st.expander("Voir les correspondances"):
                for _, tx in matches.head(5).iterrows():
                    st.markdown(f"- `{tx['label'][:50]}` → **{tx.get('category', 'Non catégorisé')}**")
                if len(matches) > 5:
...
def _render_subscriptions_manager():
    ...
    # KPIs
    total_monthly = sum(
        _estimate_monthly_amount(s.get("notes", "")) for s in confirmed
    )
...
def _render_manual_subscription_form():
    ...
    with st.form("manual_subscription"):
        ...
        notes = st.text_area("Notes", placeholder="Pourquoi ce paiement est récurrent...",
                            height=80)
        
        submitted = st.form_submit_button("✅ Ajouter comme abonnement", type="primary", 
