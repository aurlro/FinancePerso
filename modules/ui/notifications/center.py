...
def render_notification_center_full():
    ...
    with col1:
        with st.container(border=True):
            st.metric(label="Non lues", value=manager.unread_count, delta=None)

    with col2:
        with st.container(border=True):
            total = len(manager.notification_history)
            st.metric(label="Total", value=total, delta=None)
    ...
    with col3:
        with st.container(border=True):
            critical_count = len(
                [
                    n
                    for n in manager.notification_history
    ...
    with st.expander("🔍 Filtres", expanded=False):
        ...
        with col_f2:
            filter_read = st.selectbox("Statut", options=["Tous", "Non lus", "Lus"], index=0)
...
def render_notification_badge_sidebar():
    ...
    manager = get_notification_manager()
    unread = (
        manager.get_history_by_level(NotificationLevel.CRITICAL)
                ...
                for notif in unread[:5]:  # Max 5
                    with st.container(border=True):
                        ...
                        with col1:
                            ...
                            st.markdown(
                                f"<div style='font-size: 0.8em; color: #555;'>{notif.message}</div>",
                                unsafe_allow_html=True,
                            )
