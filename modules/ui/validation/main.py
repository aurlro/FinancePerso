...
from modules.db.transactions import get_pending_transactions, update_transaction_category, bulk_update_transaction_status
...
def render_validation_tab():
    ...
    with col_sort2:
        st.markdown(f"<p style='margin-top:28px; font-weight:600; color:#64748B;'>{len(df)} en attente</p>", unsafe_allow_html=True)
    ...
    @st.fragment
    def show_validation_list(filtered_df, available_categories, cat_emoji_map, sort_key, active_card_maps, key_suffix=""):
    ...
    with tab_all:
        show_validation_list(df, available_categories, cat_emoji_map, sort_key, active_card_maps, key_suffix="_all")
    
    with tab_unknown:
        unknown_mask = (df['category_validated'].fillna('Inconnu') == 'Inconnu')
        show_validation_list(df[unknown_mask], available_categories, cat_emoji_map, sort_key, active_card_maps, key_suffix="_unk")
