...
class Card:
        ...
        with st.container():
            ...
            if content:
                if isinstance(content, str):
                    st.markdown(
                        f'<div style="color: {Colors.SLATE_700}; font-size: {Typography.SIZE_BASE};">{content}</div>',
    ...
    def action(
        ...
        with st.container():
            ...
            # Bouton
            if variant == "primary":
                Button.primary(button_text, key=key, on_click=on_click, icon=button_icon)
            else:
                Button.secondary(button_text, key=key, on_click=on_click, icon=button_icon)
    ...
    def alert(
        ...
        with st.container():
            ...
            if action_text and on_action and key:
                st.markdown(
                    f'<div style="margin-top: {Spacing.SM}; margin-left: 32px;">',
                    unsafe_allow_html=True,
                )
                if severity == "error":
    ...
    def empty(
        ...
        with st.container():
            ...
            # Icône
            st.markdown(
                f'<div style="font-size: 3rem; margin-bottom: {Spacing.SM};">{icon}</div>',
                unsafe_allow_html=True,
            )
