...
def validate_theme_contrast(theme_config: dict[str, str]) -> list[ContrastRatio]:
    ...
    checks.append(
        check_contrast(
            theme_config.get("text_primary", "#1F2937"), theme_config.get("bg_card", "#FFFFFF")
        )
    )
    ...
    checks.append(
        check_contrast(
            theme_config.get("text_secondary", "#6B7280"), theme_config.get("bg_card", "#FFFFFF")
        )
    )
    ...
    checks.append(
        check_contrast(
            theme_config.get("primary", "#10B981"), theme_config.get("bg_card", "#FFFFFF")
        )
    )

    # Texte sur couleur primaire (boutons primaires)
    checks.append(
        check_contrast("#FFFFFF", theme_config.get("primary", "#10B981"))  # Texte blanc sur bouton
    )

    # Positif sur fond de carte
    checks.append(
        check_contrast(
            theme_config.get("positive", "#10B981"), theme_config.get("bg_card", "#FFFFFF")
        )
    )
    ...
    checks.append(
        check_contrast(
            theme_config.get("negative", "#EF4444"), theme_config.get("bg_card", "#FFFFFF")
        )
    )
...
if __name__ == "__main__":
    ...
    if report["failures"]:
        ...
        for f in report["failures"]:
            print(
                f"    - {f['foreground']} sur {f['background']}: {f['ratio']}:1 (requis: {f['required']}:1)"
            )
