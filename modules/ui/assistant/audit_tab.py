    ...
    with col2:
        if audit_results:
            ...
            show_hidden = st.checkbox(
                "Afficher ignorées", value=get_assistant_state("show_hidden"), key="show_hidden_cb"
            )
    ...
    if not audit_results:
        render_empty_state(
            ...
            actions=[
                (
                    "📈 Voir les tendances",
                    lambda: st.session_state.update({"active_tab": "analytics"}),
                ),
            ],
        )
        return
...
def render_bulk_actions(audit_results: list):
    ...
    with cols[3]:
        ...
        if selected_count > 0:
            if st.button(
                f"🧠 Créer règles ({selected_count})",
                use_container_width=True,
                type="primary",
                key="bulk_rules_active",
            ):
