...
def render_log_viewer():
    ...
    if not os.path.exists(LOG_FILE):
        ...
        st.info(
            "💡 Les logs apparaîtront ici après le prochain démarrage de l'application ou la prochaine action."
        )
    ...
    with col_filter2:
        search = st.text_input(
            "🔍 Recherche texte", placeholder="Error, DB, Ingestion...", key="text_input_59"
        )
    ...
    if not search:
        num_lines = st.slider(
            ...
            10,
            min(500, len(filtered_logs)),
            min(100, len(filtered_logs)),
    ...
    with col_actions2:
        if st.button(
            "🗑️ Effacer les logs", use_container_width=True, type="secondary", key="log_button_119"
        ):
