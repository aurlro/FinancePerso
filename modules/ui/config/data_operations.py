...
def render_data_operations():
    ...
    if not available_months:
        ...
        with col_d2:
            ...
            if st.button(
                "🗑️ Supprimer", type="primary", use_container_width=True, key="btn_delete_month"
            ):
                if selected_month:
                    ...
                    if not st.session_state["confirm_delete_month"]:
                        ...
                        if count > 0:
                            ...
                            show_success(
                                f"✅ {count} transaction(s) de {selected_month} ont été supprimées"
                            )
                        else:
                            show_info("ℹ️ Aucune transaction à supprimer pour cette période")
                            toast_info(
                                f"ℹ️ Aucune transaction trouvée pour {selected_month}", icon="ℹ️"
                            )
        ...
        if results.empty:
            ...
            for _, row in results.iterrows():
                with st.container(border=True):
                    ...
                    if c2.button(
                        "🗑️", key=f"del_tx_{row['id']}", help="Supprimer cette transaction"
                    ):
                        try:
    ...
    if st.button(
        "🔄 Lancer la mise à jour (Git commits)",
        use_container_width=True,
