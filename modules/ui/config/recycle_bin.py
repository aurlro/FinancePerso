...
def render_recycle_bin_manager():
    ...
    st.markdown(
        "Les transactions supprimées sont conservées 30 jours avant suppression définitive."
    )
    ...
    for _, row in df.iterrows():
        with st.container(border=True):
            ...
            with cols[3]:
                # Restore button
                if st.button(
                    "↩️ Restaurer", key=f"restore_{row['recycle_id']}", use_container_width=True
                ):
            ...
            with cols[4]:
                ...
                if st.button(
                    "🗑️ Supprimer", key=f"delete_{row['recycle_id']}", use_container_width=True
                ):
