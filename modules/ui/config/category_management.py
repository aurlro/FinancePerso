...
def render_category_management():
                ...
                with st.expander(
                    ...
                    if c1.button("💾 Mettre à jour", key=f"upd_cat_{row['id']}"):
                        try:
                            ...
                            if changes:
                                ...
                                toast_success(
                                    f"✅ '{row['name']}' mis à jour : {change_str}", icon="💾"
                                )
                            else:
                            ...
                            if "UNIQUE constraint" in error_msg:
                                toast_error(
                                    f"❌ Impossible de modifier '{row['name']}' : conflit de données",
                                    icon="⚠️",
                                )
                                ...
                                toast_error(
                                    f"❌ Erreur mise à jour de '{row['name']}' : {error_msg[:50]}",
                                    icon="❌",
                                )
                    ...
                    if c2.button("🗑️ Supprimer", key=f"del_cat_{row['id']}"):
                        try:
                            ...
                            toast_success(
                                f"🗑️ Catégorie '{row['name']}' supprimée avec succès", icon="🗑️"
                            )
                            st.rerun()
                        except Exception as e:
                            error_msg = str(e)
                            if "FOREIGN KEY" in error_msg:
                                show_error(f"Impossible de supprimer '{row['name']}'", icon="🔗")
                                toast_error(
                                ...
                                toast_warning(
                                    f"⚠️ La catégorie '{row['name']}' n'existe plus", icon="⚠️"
                                )
                                st.rerun()
                            else:
                                toast_error(
                                    f"❌ Erreur suppression '{row['name']}' : {error_msg[:50]}",
                                    icon="❌",
                                )
                ...
                if not new_cat_name or not new_cat_name.strip():
                    ...
                    try:
                        if add_category(cat_name, new_cat_emoji, int(new_is_fixed)):
                            ...
                            show_warning(f"La catégorie '{cat_name}' existe déjà", icon="⚠️")
                            toast_error(
    ...
    if len(all_cats) < 2:
        ...
        with col_m3:
            ...
            if st.button(
                "🔀 Fusionner", type="primary", use_container_width=True, key="category_button_187"
            ):
                if not source_cat or not target_cat:
                    ...
                    try:
                        ...
                        if "not found" in error_msg.lower() or "n'existe pas" in error_msg.lower():
                            toast_error(
                                f"❌ Une des catégories n'existe plus : {error_msg}", icon="⚠️"
                            )
