...
from modules.smart_reminders import get_smart_reminders, SmartReminder, ReminderType
...
def render_reminders_in_sidebar():
    ...
    with st.sidebar:
        ...
        for reminder in reminders[:2]:  # Max 2 dans la sidebar
            with st.container():
                st.write(f"**{reminder.icon} {reminder.title}**")
                st.caption(reminder.message[:60] + "..." if len(reminder.message) > 60 else reminder.message)
