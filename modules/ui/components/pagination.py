...
def paginated_dataframe(
    ...
    with col1:
        if show_page_info:
            st.caption(f"📊 {total_rows} résultats | Page {st.session_state[session_key]}/{total_pages}")
    
    with col2:
        if st.button("⏮️ Précédent", key=f"{key}_prev", disabled=st.session_state[session_key] <= 1):
            st.session_state[session_key] -= 1
            st.rerun()
    
    with col3:
        if st.button("Suivant ⏭️", key=f"{key}_next", disabled=st.session_state[session_key] >= total_pages):
            st.session_state[session_key] += 1
            st.rerun()
