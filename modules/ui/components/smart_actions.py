...
def get_user_progress_state():
    ...
    try:
        ...
        state['needs_attention'] = (
            state['uncategorized_count'] > 5 or
            (state['has_transactions'] and not state['has_rules']) or
...
def render_smart_actions():
    ...
    if state['has_transactions']:
        ...
        with col2:
            st.metric(
                label="Règles",
                value=state['rules_count'],
                label_visibility="visible"
            )
        with col3:
            st.metric(
                label="Budgets",
                value=state['budgets_count'],
                label_visibility="visible"
            )
...
def render_compact_tip():
    ...
    if state['has_transactions']:
        st.caption(
            "{} {} • {} {} • {} {}".format(
                state['tx_count'], "transactions",
                state['rules_count'], "règles",
                state['budgets_count'], "budgets"
            )
        )
