...
def render_transaction_filters(
    ...
    if show_account and "account_label" in df.columns:
        ...
        if accounts:
            sel_accounts = st.multiselect("Par Compte", accounts, key="filter_accounts")
            if sel_accounts:
    ...
    if show_member and "member" in df.columns:
        ...
        if members:
            sel_members = st.multiselect("Par Membre", members, key="filter_members")
            if sel_members:
    ...
    if show_category and "category_validated" in df.columns:
        ...
        if categories:
            sel_categories = st.multiselect("Par Catégorie", categories, key="filter_categories")
            if sel_categories:
...
def render_date_range_filter(df: pd.DataFrame, date_column: str = "date") -> pd.DataFrame:
    """
    ...
    with col1:
        start_date = st.date_input(
            "Du", value=min_date, min_value=min_date, max_value=max_date, key="filter_start_date"
        )
    with col2:
        end_date = st.date_input(
            "Au", value=max_date, min_value=min_date, max_value=max_date, key="filter_end_date"
        )
