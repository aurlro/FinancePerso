...
def render_import_tab():
    ...
    import_mode = st.radio("Format de la banque", ["BoursoBank (Auto)", "Autre (Toutes banques)"], horizontal=True)
    ...
    uploaded_file = st.file_uploader("Choisir un fichier CSV", type=['csv'], key='file_uploader_ops')
    ...
    if uploaded_file is not None:
        ...
        if ready_to_import:
            ...
            with col_q1:
                ...
                account_choice = st.radio("Compte", ["Compte existant", "Créer un nouveau compte"], key='radio_op_acc', horizontal=True)
                
                if account_choice == "Compte existant":
                    account_name = st.selectbox("Sélectionner le compte", existing_accounts, key='selectbox_op_acc')
                else:
                    account_name = st.text_input("Nom du nouveau compte", placeholder="Ex: Compte Joint, Livret A...", key='text_input_op_acc')
                    if not account_name:
            ...
            with col_q2:
                ...
                selected_year = st.selectbox("Année", years, index=len(years)-2, key='selectbox_op_year')
                ...
                if isinstance(df, tuple):
                    ...
                    if existing_hashes:
                        ...
                        if num_duplicates > 0:
                            ...
                            force_import = st.checkbox("Forcer l'import (ignorer les doublons)", value=False)
                    ...
                    if st.button("🚀 Valider et Importer", type="primary", key='btn_ops_run_import', use_container_width=True):
                        df_import = df[~duplicates_mask].copy() if existing_hashes and not force_import else df.copy()
