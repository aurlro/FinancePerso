...
def test_pattern_against_transactions(
    pattern: str, limit: int = 5, exclude_validated: bool = False
) -> tuple[list[dict], int]:
    """
...
def render_pattern_tester():
    ...
    with col1:
        test_pattern = st.text_input(
            "Pattern à tester", placeholder="Ex: UBER ou ^NETFLIX.*", key="pattern_tester_input"
        )
    ...
    with st.spinner("Analyse des transactions..."):
        matches, total = test_pattern_against_transactions(
            test_pattern, limit=5, exclude_validated=only_pending
        )
    ...
    if total == 0:
        ...
        if only_pending:
            st.caption(
                "💡 Essayez de décocher 'Non catégorisées uniquement' pour voir toutes les transactions."
            )
    else:
        ...
        if matches:
            ...
            for match in matches:
                ...
                with cols[1]:
                    st.markdown(
                        f"{match['label'][:40]}..." if len(match["label"]) > 40 else match["label"]
                    )
                with cols[2]:
                    amount = match["amount"]
