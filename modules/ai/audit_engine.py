...
def ai_audit_batch(df: pd.DataFrame, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for sugg in suggestions:
            ...
            # Find matching rows
            matches = df[
                (df['clean'] == clean_label_val) & 
                (df['category_validated'] == current_cat)
            ]
...
def run_full_audit(df: pd.DataFrame, use_ai: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        results['total_anomalies'] = len(results['inconsistencies']) + len(results['ai_suggestions'])
...
def get_audit_summary(audit_results: List[Dict], 
                     corrected: List[int], 
                     hidden: List[int]) -> Dict[str, int]:
    """
