...
def get_budget_status(category: str = None) -> dict:
    ...
    if category:
        ...
        return {"message": "Budgets disponibles", "categories": budgets["category"].tolist()}
...
def chat_with_assistant(user_message: str, conversation_history: list = None) -> str:
    ...
    try:
        ...
        messages = [f"Question utilisateur : {user_message}"]
        ...
        while current_turn < max_turns:
            ...
            if json_match:
                try:
                    ...
                    messages.append(
                        f"System (Tool Result): {json.dumps(result, ensure_ascii=False)}"
                    )
                    current_turn += 1
                    continue
