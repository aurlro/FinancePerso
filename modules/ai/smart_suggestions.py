...
class SmartSuggestionEngine:
    ...
    def _analyze_frequent_patterns(self):
        ...
        existing_patterns = (
            set(self.rules["pattern"].str.lower().tolist()) if not self.rules.empty else set()
        )
    ...
    def _analyze_potential_duplicates(self):
        ...
        grouped = (
            self.transactions.groupby(["date", "label", "amount"]).size().reset_index(name="count")
        )
    ...
    def _analyze_missing_tags(self):
        ...
        untagged = self.transactions[
            (self.transactions["tags"].isna()) | (self.transactions["tags"] == "")
        ]
