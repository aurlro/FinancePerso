...
class CategoryInsightsEngine:
    ...
    def get_category_insights(self, category: str, current_month_df: pd.DataFrame) -> List[CategoryInsight]:
        """
    ...
    def _detect_amount_anomalies(self, category: str, history: pd.DataFrame, 
                                 current: pd.DataFrame) -> List[CategoryInsight]:
    ...
    def _analyze_trend(self, category: str, history: pd.DataFrame, 
                      current: pd.DataFrame) -> List[CategoryInsight]:
    ...
    def _detect_new_merchants(self, category: str, history: pd.DataFrame, 
                             current: pd.DataFrame) -> List[CategoryInsight]:
    ...
    def _generate_savings_suggestions(self, category: str, history: pd.DataFrame, 
                                     current: pd.DataFrame) -> List[CategoryInsight]:
    ...
    def get_top_insights(self, current_month_df: pd.DataFrame, max_insights: int = 5) -> List[CategoryInsight]:
        """
        ...
        all_insights.sort(key=lambda x: (
            ...
            -(x.savings_potential or 0)
        ))
        
        return all_insights[:max_insights]
    
    def get_anomalous_transactions(self, category: str, current_month_df: pd.DataFrame) -> List[TransactionAnomaly]:
        """
...
def render_category_insights_card(insight: CategoryInsight):
    ...
    with st.container(border=True):
        ...
        with col2:
            ...
            if insight.action:
                if st.button(insight.action, key=f"insight_{insight.category}_{insight.insight_type}", 
