...
Migrated from: modules/ui/components/tag_selector_smart.py
Classification: Molecule (smart composite with propagation logic)
"""
...
from modules.db.transactions_batch import bulk_tag_transactions
...
def __get_transaction_by_id(tx_id: int) -> dict[str, Any] | None:
    """Get a single transaction by ID."""
    with get_db_connection() as conn:
        import pandas as pd
        df = pd.read_sql("SELECT * FROM transactions WHERE id = ?", conn, params=(tx_id,))
        if df.empty:
            return None
...
def find_similar_transactions(
    ...
    source = _get_transaction_by_id(transaction_id)
    if not source:
        return []
...
def apply_tag_to_similar(
    ...
    for trans in similar_transactions:
        ...
        # Get current tags
        trans_data = _get_transaction_by_id(trans_id)
        if not trans_data:
            continue
        ...
        if tag not in current_tags:
            ...
            bulk_tag_transactions([trans_id], new_tags)
            count += 1
...
def render_tag_propagation_dialog(
    ...
    with st.expander(
        f"🔄 Propager à {count} transaction{'s' if count > 1 else ''} similaire", expanded=True
    ):
...
def render_smart_tag_selector(
    ...
    all_available = sorted(
        list(set(existing_tags + suggested + st.session_state["temp_custom_tags"] + current_tags))
    )
            ...
            for new_tag in new_tags:
                ...
                if propagation_key not in st.session_state["propagation_shown"]:
                    ...
                    similar = find_similar_transactions(
                        transaction_id, "", category, df_context  # Label will be fetched internally
                    )

                    if similar:
                        render_tag_propagation_dialog(new_tag, transaction_id, similar, key_suffix)
            ...
            with st.popover("➕", use_container_width=True):
                ...
                if st.button(
                    ...
                    if new_tag and new_tag.strip():
                        ...
                        st.session_state["pending_tag_additions"][transaction_id].append(
                            new_tag_clean
                        )
                        st.rerun()
