...
Migrated from: modules/ui/components/quick_actions.py
Classification: Molecule (composite component with multiple popovers and forms)
"""
...
from modules.db.transactions import bulk_update_transaction_status, get_pending_transactions, save_transactions
...
from modules.ui_v2.molecules.toasts import toast_error
from modules.ui_v2.molecules.banners import show_info, show_warning
from modules.ui_v2.organisms.flash_messages import set_flash_message, FlashType
...
def render_quick_validation_popover():
    ...
    with st.popover("🧠 Validation rapide", use_container_width=True):
        ...
        if df.empty:
            from modules.ui_v2.molecules.components.empty_states import render_no_transactions_state
            ...
            with st.container(border=True):
                ...
                with cols[3]:
                    if st.button("✓", key=f"quick_val_{row['id']}", type="primary"):
                        ...
                        validated_count += 1
                        set_flash_message(
                            f"✅ Transaction validée : {row['label'][:30]}...", FlashType.SUCCESS
                        )
                        st.rerun()
                ...
                if submitted:
                    if new_name and new_name.strip():
                        ...
                        if add_member(cleaned_name, new_type):
                            ...
                            set_flash_message(
                                f"⚠️ Le membre '{cleaned_name}' existe déjà", FlashType.WARNING
                            )
                    else:
                        set_flash_message("⚠️ Veuillez entrer un nom", FlashType.WARNING)
                ...
                if submitted:
                    if new_cat and new_cat.strip():
                        ...
                        if add_category(cleaned_cat, emoji, int(is_fixed)):
                            ...
                            set_flash_message(
                                f"⚠️ La catégorie '{cleaned_cat}' existe déjà", FlashType.WARNING
                            )
                    else:
                        set_flash_message(
                            "⚠️ Veuillez entrer un nom de catégorie", FlashType.WARNING
                        )
                ...
                if submitted:
                    if pattern and pattern.strip():
                        ...
                        try:
                            ...
                            set_flash_message(
                                f"❌ Erreur création règle : {str(e)[:50]}", FlashType.ERROR
                            )
                    else:
                        set_flash_message("⚠️ Veuillez entrer un mot-clé", FlashType.WARNING)
            ...
            try:
                ...
                if st.button(
                    ...
                    if count > 0:
                        ...
                        set_flash_message(msg, FlashType.SUCCESS)
                    else:
                        set_flash_message(
                            "ℹ️ Aucune nouvelle transaction à importer", FlashType.INFO
                        )
