...
Migrated from: modules/ui/components/daily_widget.py
Classification: Molecule (daily insight widget)
"""
    ...
    try:
        ...
        if not budgets_df.empty:
            ...
            spending_by_category = (
                month_tx.groupby("category")["amount"].sum().to_dict() if not month_tx.empty else {}
            )
    ...
    with st.container():
        ...
        with col2:
            ...
            if "progress" in insight:
                st.progress(value=min(insight["progress"], 1.0), text=f"{insight['progress']:.0%}")
...
def render_quick_stats_row():
    ...
    with cols[2]:
        ...
        st.metric(label="📥 Dernier import", value=last_import if last_import else "Aujourd'hui")
