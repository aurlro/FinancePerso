...
Migrated from: modules/ui/components/savings_goals_widget.py
Classification: Molecule (widget with progress and interactions)
"""
...
def render_savings_goal_card(goal: SavingsGoal, compact: bool = False):
    ...
    if compact:
        ...
        with st.container(border=True):
            ...
            with col1:
                st.markdown(
                    f"<h1 style='text-align: center; margin: 0;'>{goal.emoji}</h1>",
            ...
            if not goal.is_achieved():
                with st.form(f"contribute_{goal.id}"):
                    ...
                    submit = st.form_submit_button(
                        "💰 Contribuer", type="primary", use_container_width=True
                    )
                    ...
                    if submit:
                        ...
                        if amount > 0:
                            if contribute_to_savings_goal(goal.id, amount):
                                toast_success(
                                    f"✅ {amount:.0f}€ ajoutés à '{goal.name}' !", icon="💰"
                                )
                                ...
                                if goal.current_amount + amount >= goal.target_amount:
                                    st.balloons()
                                    st.success(
                                        f"🎉 Félicitations ! Objectif '{goal.name}' atteint !"
                                    )
...
def render_goal_creation_form():
    ...
    with st.form("create_goal_form"):
        ...
        description = st.text_area(
            "Description (optionnel)", placeholder="Pourquoi cet objectif est important pour vous ?"
        )
        ...
        with col3:
            target = st.number_input(
                "Montant objectif (€)", min_value=100.0, value=1000.0, step=100.0
            )

        with col4:
            initial = st.number_input(
                "Montant déjà épargné (€)", min_value=0.0, value=0.0, step=50.0
            )

        deadline = st.date_input("Date cible (optionnel)", value=None, min_value=st.date_input)

        submitted = st.form_submit_button(
            "✨ Créer l'objectif", type="primary", use_container_width=True
        )
...
def render_savings_goals_page():
    ...
    # Import manager for achieved goals
    ...
    if achieved:
        ...
        with st.expander("Voir les objectifs complétés"):
            for goal in achieved:
                st.success(
                    f"{goal.emoji} **{goal.name}** - {goal.target_amount:.0f}€ atteints ! 🎉"
                )


# Helper pour l'intégration dans le daily widget
def get_goal_insight_for_daily_widget() -> dict | None:
    ...
    if progress >= 90:
        ...
        message = "💪 À mi-parcours ! Continuez comme ça"
        priority = "medium"
    else:
