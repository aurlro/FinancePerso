...
from modules.ui_v2.molecules.banners import show_error, show_success
...
def load_env_vars() -> Dict[str, str]:
    ...
    if os.path.exists(env_file):
        try:
            ...
            st.error(f"{IconSet.ERROR.value} Erreur lors de la lecture du fichier .env: {e}")
...
def validate_api_key(key: str, provider: str) -> bool:
    ...
    Args:
        ...
        provider: The provider name (Gemini, OpenAI, DeepSeek)
...
def set_secure_env_permissions(env_file: str) -> None:
    """
    ...
    try:
        ...
        toast_warning(f"{IconSet.WARNING.value} Impossible de définir les permissions sécurisées: {e}", icon=IconSet.LOCKED.value)
...
def render_api_settings() -> None:
    ...
    """
    st.header(f"{IconSet.SETTINGS.value} Cerveau IA")
    ...
    with st.form("ai_config"):
        ...
        if selected_provider == "Gemini":
            st.info(f"{IconSet.INFO.value} Le modèle standard : Rapide, performant et gratuit/pas cher.")
            val = st.text_input(
            ...
            st.info(f"{IconSet.LOCKED.value} 100% Local & Privé. Assurez-vous qu'Ollama tourne sur votre machine.")
            url = st.text_input(
            ...
            st.info(f"{IconSet.INFO.value} Performance coût/efficacité redoutable.")
            val = st.text_input(
                "Clé API DeepSeek",
            ...
            st.info(f"{IconSet.INFO.value} Le standard de l'industrie (GPT-4 / GPT-3.5).")
            val = st.text_input(
                "Clé API OpenAI",
        ...
        st.subheader(f"{IconSet.SETTINGS.value} Options Avancées")
        ...
        submitted = st.form_submit_button(
            f"{IconSet.SAVE.value} Sauvegarder et Appliquer", type="primary", use_container_width=True
        )
        ...
        if submitted:
            ...
            if validation_error:
                show_error(validation_error, icon=IconSet.SETTINGS.value)
                toast_error(f"{IconSet.ERROR.value} {validation_error}", icon=IconSet.SETTINGS.value)
            else:
                try:
                    env_file = ".env"
                    ...
                    show_success(f"Configuration {selected_provider} enregistrée")

                    # Toast détaillé
                    config_msg = f"{IconSet.SUCCESS.value} Configuration {selected_provider} enregistrée"
                    if model_val:
                        config_msg += f" (modèle: {model_val})"
                    toast_success(config_msg, icon=IconSet.SETTINGS.value)

                    show_success(
                        f"{IconSet.SETTINGS.value} Configuration IA mise à jour ! Fournisseur : {selected_provider}"
                    )
                    toast_info(f"{IconSet.LOCKED.value} Permissions sécurisées appliquées au fichier .env", icon=IconSet.LOCKED.value)

                    # Afficher un récapitulatif
                    with st.expander(f"{IconSet.INFO.value} Récapitulatif de la configuration", expanded=False):
                        ...
                        st.success(f"{IconSet.SUCCESS.value} Les changements sont immédiatement actifs")
                    ...
                    error_msg = str(e)
                    show_error(f"Configuration {selected_provider} enregistrée", success=False)
                    show_error(f"Erreur lors de la sauvegarde : {e}", icon=IconSet.ERROR.value)
                    toast_error(f"{IconSet.ERROR.value} Échec de la sauvegarde : {error_msg[:80]}", icon=IconSet.ERROR.value)

    # Info section
    st.divider()
    with st.expander(f"{IconSet.INFO.value} Guide de configuration", expanded=False):
        st.markdown(
            """
