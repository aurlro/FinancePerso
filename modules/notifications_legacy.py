    ...
    def add(self, notification):
        ...
        if hasattr(notification, "message"):
            ...
            self._v2_manager.notify(
                ...
                level=level,
                actions=(
                    [NotificationAction(label="Voir", url=notification.action_url)]
                    if hasattr(notification, "action_url") and notification.action_url
                    else []
                ),
            )
            ...
            for _, budget in budgets_df.iterrows():
                ...
                if level:
                    ...
                    manager.notify(
                        ...
                        actions=[
                            NotificationAction(label="Voir Budgets", url="pages/03_Intelligence.py")
                        ],
                    )
...
def generate_daily_digest() -> bool:
    ...
    manager.notify(
        ...
        level=NotificationLevel.INFO,
        group="daily_digest",
        actions=[NotificationAction(label="Synthèse", url="pages/02_Dashboard.py")],
    )
    return True
...
MyFinance Companion"""
                ...
                try:
                    ...
                    results["errors"].append(
                        f"Impossible de se connecter au serveur SMTP: {str(e)}"
                    )
                except Exception as e:
                    results["errors"].append(f"Erreur SMTP: {str(e)}")
                finally:
...
class NotificationManager:
    ...
    def clear_old(self, days: int = 30):
        ...
        self.notifications = [
            n for n in self.notifications if datetime.fromisoformat(n.created_at) > cutoff
        ]
        self._save()
...
Continuez à suivre vos objectifs ! 📊"""
    ...
    notif = Notification(
        ...
        action_url="pages/02_Dashboard.py",
    )
