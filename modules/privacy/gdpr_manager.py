...
class GDPRManager:
    ...
    def purge_user_data(
        ...
        logger.info(f"PURGE complétée pour user {user_id}: {total_records} enregistrements supprimés")
    ...
    def export_user_data(self, user_id: str, format: str = "json") -> Dict[str, Any]:
        ...
        logger.info(f"Export données pour user {user_id}: {len(export_data['transactions'])} transactions")
    ...
    def _user_exists(self, user_id: str) -> bool:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) FROM settings WHERE user_id = ?",
                (user_id,)
            )
    ...
    def _save_deletion_log(self, record: DeletionRecord):
        ...
        log_file = log_dir / f"deletion_{record.user_id}_{record.deleted_at.strftime('%Y%m%d')}.json"
