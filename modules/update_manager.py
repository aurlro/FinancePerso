...
class UpdateManager:
    ...
    def _format_changelog_entry(self, entry: VersionEntry) -> str:
        ...
        lines = [f"## [{entry.version}] - {entry.date}", "", f"### {entry.title}", ""]
    ...
    def update_agents_md(self, entry: VersionEntry) -> bool:
        ...
        try:
            ...
            if re.search(version_pattern, content):
                content = re.sub(
                    version_pattern, f"**Version du projet**: `v{entry.version}`", content
                )
            else:
            ...
            if re.search(date_pattern, content):
                content = re.sub(date_pattern, f"**Dernière mise à jour** : {entry.date}", content)
            ...
            if "added" in entry.categories and entry.categories["added"]:
                ...
                feature_section = re.search(
                    r"### Fonctionnalités principales.*?(?=###|\Z)", content, re.DOTALL
                )
                if feature_section:
                    # Add new features
    ...
    def update_constants(self, version: str) -> bool:
        ...
        try:
            ...
            if "APP_VERSION" in content:
                content = re.sub(
                    r'APP_VERSION\s*=\s*["\'][^"\']+["\']', f'APP_VERSION = "{version}"', content
                )
            else:
                content += f'\nAPP_VERSION = "{version}"\n'
            ...
            if recent:
                ...
                if current_items:
                    ...
                    if match_ratio > 0.8:  # 80% similarity threshold
                        logger.warning(
                            f"Duplicate update detected: {match_ratio:.0%} similar to v{last_entry.get('version')}"
                        )
                        return (
                            False,
                            f"Cette mise à jour est identique à la version précédente (v{last_entry.get('version')}). Cochez 'Forcer la création' pour ignorer cette alerte.",
                        )
    ...
    def analyze_git_changes(self) -> dict:
        ...
        try:
            ...
            if status_result.returncode == 0 and status_result.stdout.strip():
                ...
                result["uncommitted_status"] = {
                    k: v for k, v in uncommitted_status.items() if k in result["uncommitted_files"]
                }
    ...
    def get_module_changes(self) -> dict:
        ...
        """
        result = {"files_modified": [], "added": [], "suggested_title": "Mise à jour des modules"}
...
def quick_update(title: str, changes: list[str], bump_type: str = "patch") -> tuple[bool, str]:
    ...
    return manager.create_update(title=title, changes={"added": changes}, bump_type=bump_type)
