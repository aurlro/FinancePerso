...
class DynamicBudgetEngine:
    ...
    def analyze_seasonality(self, category: str, min_months: int = 6) -> Optional[SeasonalPattern]:
        ...
        confidence = min(1.0, variance * 2)  # Plus la variance est élevée, plus le pattern est marqué
    ...
    def suggest_budget_adjustments(self) -> List[BudgetAdjustment]:
        ...
        for _, budget_row in budgets.iterrows():
            ...
            if diff_pct > 0.2:  # > 20% d'écart
                ...
                suggestions.append(BudgetAdjustment(
                    category=category,
            ...
            if remaining > 0:
                ...
                if actual_progress < expected_progress * 0.8:
                    results.append({
                        ...
                        'daily_allowance': remaining / (days_in_month - day_of_month) if day_of_month < days_in_month else 0,
...
class BudgetChallenge:
    ...
    @classmethod
    def get_available_challenges(cls, df_history: pd.DataFrame, current_month_df: pd.DataFrame) -> List[Dict]:
        """
