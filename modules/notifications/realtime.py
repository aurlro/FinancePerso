    ...
    def check_savings_milestone(
        ...
        if previous_savings <= 0 and current_savings > 0:
            ...
            alert = RealTimeAlert(
                ...
                actions=[{"label": "Voir le détail", "action": "view_savings"}],
            )
            self.alerts.append(alert)
            self._save_alerts()
            return alert
        ...
        if previous_savings > 0:
            ...
            if improvement >= 50:
                alert = RealTimeAlert(
                    ...
                    amount=current_savings,
                    actions=[{"label": "Continuer ainsi !", "action": "view_trends"}],
                )
                self.alerts.append(alert)
                self._save_alerts()
                return alert
...
def render_notification_banner():
    ...
    if critical_alerts:
        with st.container(border=True):
            st.markdown(
                f"### 🔔 {len(critical_alerts)} alerte{'s' if len(critical_alerts) > 1 else ''} en attente"
            )
