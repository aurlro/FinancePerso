...
def detect_recurring_transactions(
    ...
    try:
        ...
        df['label_key'] = df['label'].str.extract(r'(\w+)') + '_' + df['amount'].abs().round(2).astype(str)
        ...
        for label_key, group in df.groupby('label_key'):
            ...
            if avg_interval > 0:
                ...
                confidence = regularity * min(len(group) / 6, 1)  # More occurrences = higher confidence
            else:
                confidence = 0
            ...
            if freq == RecurringFrequency.MONTHLY:
                day_of_month = int(group['day'].mode().iloc[0]) if len(group['day'].mode()) > 0 else None
            ...
            recurring.append(RecurringTransaction(
                ...
                category=group['category_validated'].mode().iloc[0] if len(group['category_validated'].mode()) > 0 else None
            ))
