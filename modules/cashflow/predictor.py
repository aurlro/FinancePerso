...
def predict_monthly_cashflow(months_ahead: int = 3) -> list[CashflowPrediction]:
    ...
    try:
        ...
        for i in range(months_ahead):
            ...
            monthly_avg_income = (
                df[df["amount"] > 0].groupby(df["date"].dt.month)["amount"].sum().mean()
            )
            monthly_avg_expense = (
                df[df["amount"] < 0].groupby(df["date"].dt.month)["amount"].sum().mean()
            )
...
def predict_account_balance(target_date: datetime) -> dict:
    ...
    try:
        ...
        months_ahead = (
            (target_date.year - datetime.now().year) * 12
        ...
        for pred in predictions:
            if (
                pred.start_date.month == target_date.month
                and pred.start_date.year == target_date.year
            ):
                return {
...
def get_cashflow_insights() -> dict:
    ...
    try:
        ...
        insights = {
            ...
            "predicted_3m_balance": (
                predictions[-1].predicted_balance if len(predictions) >= 3 else None
            ),
            "trend": (
                "up"
                if predictions[-1].predicted_balance > predictions[0].starting_balance
                else "down"
            ),
            "warnings": [],
            "avg_monthly_income": sum(p.predicted_income for p in predictions) / len(predictions),
            "avg_monthly_expense": abs(
                sum(p.predicted_expenses for p in predictions) / len(predictions)
            ),
        }
