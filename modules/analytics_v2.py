...
def detect_recurring_payments_v2(df: pd.DataFrame) -> pd.DataFrame:
    ...
    data["income_check"] = data.apply(
        ...
        or detect_income_pattern(x["label"])[0],
        axis=1,
    )
...
def _analyze_group(
    ...
    current_cat = (
        ...
        if not group["category_validated"].empty
        else "Non catégorisé"
    )
    ...
    return {
        ...
        "variability": (
            "Variable" if variability_ratio > AMOUNT_TOLERANCE_FIXED_THRESHOLD else "Fixe"
        ),
...
def get_recurring_by_tags(df: pd.DataFrame, recurring_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Group by tag
    grouped = (
        tags_exploded.groupby("tag_list")
        .agg({"amount": ["sum", "mean", "count"], "id": lambda x: list(x.unique())})
        .reset_index()
    )
...
def analyze_recurrence_summary(df: pd.DataFrame, recurring_df: pd.DataFrame) -> Dict:
    ...
    summary = {
        ...
        "monthly_expense_total": (
            abs(
                expenses[expenses["frequency_label"] == FREQUENCY_MONTHLY_LABEL]["avg_amount"].sum()
            )
            if not expenses.empty
            else 0
        ),
        "monthly_income_total": (
            incomes[incomes["frequency_label"] == FREQUENCY_MONTHLY_LABEL]["avg_amount"].sum()
