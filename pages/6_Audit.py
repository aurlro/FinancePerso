...
if active_tab == "📊 Dashboard Santé":
    ...
    def calculate_health_scores():
        ...
        if not rules_df.empty:
            try:
                ...
                scores["rules"] = {
                    ...
                    "status": "✅" if audit.get("score", 0) >= 80 else ("⚠️" if audit.get("score", 0) >= 50 else "🚨"),
        ...
        if not transactions_df.empty:
            ...
            if "category" in transactions_df.columns:
                uncategorized = len(transactions_df[transactions_df["category"].isna() | (transactions_df["category"] == "")])
                completeness_issues += uncategorized
            
            # Check for missing members
            if "member" in transactions_df.columns:
                unassigned = len(transactions_df[transactions_df["member"].isna() | (transactions_df["member"] == "")])
                completeness_issues += unassigned
            
            completeness_score = max(0, 100 - completeness_issues)
            scores["completeness"] = {
                "score": completeness_score,
                "status": "✅" if completeness_score >= 95 else ("⚠️" if completeness_score >= 80 else "🚨"),
    ...
    if actions:
        ...
        for action in actions:
            priority_color = {"high": "🔴", "medium": "🟡", "low": "🟢"}.get(action["priority"], "⚪")
            
            with st.expander(f"{priority_color} {action['icon']} {action['title']}", expanded=(action["priority"] == "high")):
                ...
                with col1:
                    if action.get("link"):
                        if action["link"].startswith("?"):
                            if st.button(action["action"], key=f"action_{action['title'][:20]}", type="primary"):
                                st.session_state["audit_active_tab"] = action["link"].replace("?tab=", "").replace("+", " ")
                                st.rerun()
                        else:
...
# =============================================================================
    ...
    if transactions_df.empty:
        ...
        with col3:
            ...
            st.metric("En attente", pending, delta="à traiter" if pending > 0 else None, delta_color="inverse")
        with col4:
            if "category" in transactions_df.columns:
                uncategorized = len(transactions_df[transactions_df["category"].isna() | (transactions_df["category"] == "")])
                st.metric("Sans catégorie", uncategorized, delta="à catégoriser" if uncategorized > 0 else None, delta_color="inverse")
        ...
        # Check 1: Duplicates
        if not transactions_df.empty and "date" in transactions_df.columns and "label" in transactions_df.columns:
        ...
        if "member" in transactions_df.columns:
            unassigned = transactions_df[transactions_df["member"].isna() | (transactions_df["member"] == "")]
            if len(unassigned) > 0:
                checks.append({
                    "status": "info",
        ...
        for check in checks:
            ...
            with st.container(border=True):
                ...
                if check.get("action"):
                    st.button(check["action"], key=f"check_{check['title'][:20]}", use_container_width=True)
