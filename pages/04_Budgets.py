...
from modules.ui.rules.budget_manager import render_budget_section
...
if active_tab == "📊 Vue d'ensemble":
    ...
    if budgets_df.empty or transactions_df.empty:
        ...
        WelcomeEmptyState.render(
            ...
            secondary_action_link="pages/01_Import.py",
            show_steps=False,
        )
    else:
        ...
        if not transactions_df.empty and "date" in transactions_df.columns:
                ...
                budget_comparison.append(
                    {
                        ...
                        "État": (
                            "⚠️ Dépassement"
                            if pct_used > 100
                            else ("🟡 Attention" if pct_used > 80 else "✅ OK")
                        ),
                    }
                )
            ...
            if budget_comparison:
                ...
                with col_chart:
                    ...
                    colors = [
                        "red" if x > 100 else ("orange" if x > 80 else "green")
    ...
    if budgets_df.empty:
        ...
        with col2:
            ...
            notify_daily = st.toggle(
                "Résumé quotidien", value=True, help="Afficher les alertes dans le widget du jour"
            )

            notify_weekly = st.toggle(
                "Résumé hebdomadaire", value=False, help="Recevoir un récapitulatif chaque semaine"
            )
        ...
        if not transactions_df.empty and not budgets_df.empty:
            ...
            st.info(
                "Importez des transactions et définissez des budgets pour recevoir des conseils personnalisés."
            )
