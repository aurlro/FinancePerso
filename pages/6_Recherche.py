...
if active_tab == "🔍 Recherche Globale":
    ...
    explorer_type = st.session_state.pop("_explorer_type", None) or st.query_params.get(
        "type", "category"
    )
    explorer_value = st.session_state.pop("_explorer_value", None) or st.query_params.get(
        "value", ""
    )
    from_page = st.session_state.pop("_explorer_from", None) or st.query_params.get(
        "from", "6_Explorer"
    )
    ...
    if not explorer_value:
        ...
        render_explorer_page(
            explorer_type=explorer_type, explorer_value=explorer_value, from_page=from_page
        )
