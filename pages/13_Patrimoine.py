...
from src.wealth_manager import (
    WealthManager, RealEstateAsset, FinancialAsset, CryptoAsset,
    Liability, MortgageSchedule, AssetType, LiabilityType,
    calculate_debt_to_income_ratio, calculate_monthly_debt_service,
)
from src.agent_core import (
    AgentOrchestrator, Mission, MissionPriority, MissionStatus,
    ActionType, quick_analyze,
)
from src.subscription_engine import Subscription
                ...
                with col2:
                    ...
                    if st.button("Exécuter", key=btn_key):
                        ...
                        if action.type.value == "generate_letter":
                            ...
                            # Créer le document
                            from src.agent_core import DocumentGenerator
                            ...
                            st.markdown(f"[🌐 Ouvrir le comparateur](https://comparateur.example.com/{category})")
                            ...
                            for sugg in suggestions:
                                st.write(f"- **{sugg['type']}**: {sugg['yield']} (risque: {sugg['risk']})")
...
def _render_real_estate_tab(wealth_manager: WealthManager):
    ...
    for asset in wealth_manager.real_estate:
        with st.expander(f"🏠 {asset.name}", expanded=True):
            ...
            if asset.mortgage and st.checkbox("Voir tableau d'amortissement", key=f"amort_{asset.id}"):
                df = asset.mortgage.schedule_df.head(24)  # 24 premiers mois
                st.dataframe(
                    df[['month', 'payment', 'principal_part', 'interest_part', 'remaining_balance']],
                    ...
                    column_config={
                        ...
                        'remaining_balance': st.column_config.NumberColumn("Restant dû", format="€%.2f"),
                    }
                )
